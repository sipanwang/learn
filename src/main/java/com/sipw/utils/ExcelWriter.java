...
public class ExcelWriter {
    ...
    static{
        ...
        CELL_HEADS.add("核查单编号");
        CELL_HEADS.add("主体号 (操作 / 支付方)");
        CELL_HEADS.add("收款方主体号");
        CELL_HEADS.add("收款方名称");
        CELL_HEADS.add("交易时间");
    }
    ...
    private static void convertDataToRow(ExcelDataVO data, Row row){
        ...
        // 主体号 (操作 / 支付方)
        cell = row.createCell(cellNum++);
        cell.setCellValue(null==data.getPayCustId() ?"":data.getPayCustId());
        // 收款方主体号
        cell = row.createCell(cellNum++);
        cell.setCellValue(null == data.getRecCustId() ? "" : data.getRecCustId());
        // 收款方名称
        cell = row.createCell(cellNum++);
        cell.setCellValue(null == data.getRecCustName() ? "" : data.getRecCustName());
        //交易时间,date转String
        cell = row.createCell(cellNum++);
